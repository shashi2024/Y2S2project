...
const orderSchema = new mongoose.Schema({
    ...
    items: [{ foodItemID: { type: mongoose.Schema.Types.ObjectId, ref: 'FoodItem' }, quantity: Number, specialRequests: String }],
...
orderSchema.post('save', async function decreaseInventoryQuantity(doc, next) {
    const updatePromises = doc.items.map(async (item) => {
        const inventoryItem = await RestaurantInventoryItem.findOne({ foodItem: item.foodItemID });
        if (!inventoryItem) {
