...
function ShowStaff() {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`http://localhost:5000/staff/${id}`)
      .then((res) => {
    ...
    <div className="flex items-center justify-center h-screen bg-red-200">
      <div className="bg-white p-8 rounded shadow-lg" ref={componentRef}>
        ...
        <div className="space-y-4">
          <div>
            <div className="font-semibold text-indigo-500">staff Name</div>
            <p className="text-gray-500">{staff.name}</p>
          </div>
          <div>
            <div className="font-semibold text-indigo-500">staff ID</div>
            <p className="text-gray-500">{staff.sID}</p>
          </div>
          <div>
            <div className="font-semibold text-indigo-500">staff Email</div>
          ...
          <div>
            <div className="font-semibold text-indigo-500">NIC</div>
            <p className="text-gray-500">{staff.nic}</p>
          </div>
          <div>
            <div className="font-semibold text-indigo-500">Position</div>
